/**
 * seed.mjs — Create and populate KNOWLEDGE_GRAPH/math-kg.db
 * Run: node KNOWLEDGE_GRAPH/seed.mjs
 */
...
const require = createRequire(
  path.join(__dirname, '..', 'seraph-field-site', 'package.json')
);
const Database = require('better-sqlite3');

// ── DB path ──
const DB_PATH = path.join(__dirname, 'math-kg.db');
...
CREATE TABLE edge_types (
  id TEXT PRIMARY KEY, label TEXT NOT NULL, color TEXT NOT NULL, dash TEXT
);
CREATE TABLE tag_groups (
  id TEXT PRIMARY KEY, label TEXT NOT NULL, color TEXT NOT NULL
);
CREATE TABLE tags (
  id TEXT PRIMARY KEY, group_id TEXT NOT NULL REFERENCES tag_groups(id), name TEXT NOT NULL
);
CREATE TABLE nodes (
  id TEXT PRIMARY KEY, label TEXT NOT NULL,
  type TEXT NOT NULL CHECK(type IN ('definition', 'instance')),
  category TEXT, desc TEXT, detail_ref TEXT
);
CREATE TABLE edges (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  source TEXT NOT NULL REFERENCES nodes(id),
  target TEXT NOT NULL REFERENCES nodes(id),
  type TEXT NOT NULL REFERENCES edge_types(id),
  label TEXT, detail TEXT, detail_ref TEXT,
...
const insertTagGroup = db.prepare(
  'INSERT INTO tag_groups (id, label, color) VALUES (?, ?, ?)'
);
insertTagGroup.run('map',     '사상 (map)',       '#ff9966');
insertTagGroup.run('functor', '함자 (functor)',   '#ffcc44');
insertTagGroup.run('natural', '자연변환 (natural)', '#66ddff');

// ── Tags ──
const insertTag = db.prepare(
  'INSERT INTO tags (id, group_id, name) VALUES (?, ?, ?)'
);
insertTag.run('functor:free',       'functor', 'free');
insertTag.run('functor:forgetful',  'functor', 'forgetful');
insertTag.run('functor:lie',        'functor', 'lie');
insertTag.run('functor:dual',       'functor', 'dual');
insertTag.run('map:inclusion',      'map',     'inclusion');
insertTag.run('map:projection',     'map',     'projection');

// ── Nodes ──
const insertNode = db.prepare(
  'INSERT INTO nodes (id, label, type, category, desc, detail_ref) VALUES (?, ?, ?, ?, ?, ?)'
);

const nodes = [
  { id: "Set",            label: "집합",             type: "definition", category: "algebra",  desc: "원소들의 모임. 가장 기본적인 수학 구조." },
  { id: "Monoid",         label: "모노이드",         type: "definition", category: "algebra",  desc: "결합법칙을 만족하는 이항연산과 항등원을 가진 집합." },
  { id: "Group",          label: "군 (Group)",       type: "definition", category: "algebra",  desc: "모노이드에 역원이 추가된 대수 구조." },
  { id: "AbelianGroup",   label: "아벨군",           type: "definition", category: "algebra",  desc: "교환법칙을 만족하는 군." },
  { id: "Ring",           label: "환 (Ring)",        type: "definition", category: "algebra",  desc: "덧셈에 대해 아벨군, 곱셈에 대해 모노이드인 구조." },
  { id: "Field",          label: "체 (Field)",       type: "definition", category: "algebra",  desc: "0이 아닌 모든 원소가 곱셈의 역원을 가지는 가환환." },
  { id: "Module",         label: "가군 (Module)",    type: "definition", category: "algebra",  desc: "환 위의 벡터공간 일반화." },
  { id: "VectorSpace",    label: "벡터공간",         type: "definition", category: "linalg",   desc: "체 위의 가군." },
  { id: "Algebra_str",    label: "대수 (Algebra)",   type: "definition", category: "algebra",  desc: "벡터공간이면서 동시에 환인 구조 (결합대수)." },
  { id: "TopSpace",       label: "위상공간",         type: "definition", category: "geometry", desc: "열린집합의 체계가 정의된 집합." },
  { id: "MetricSpace",    label: "거리공간",         type: "definition", category: "geometry", desc: "거리함수가 정의된 집합." },
  { id: "NormedSpace",    label: "노름공간",         type: "definition", category: "analysis", desc: "노름이 정의된 벡터공간." },
  { id: "BanachSpace",    label: "바나흐 공간",      type: "definition", category: "analysis", desc: "완비 노름공간." },
  { id: "HilbertSpace",   label: "힐베르트 공간",    type: "definition", category: "analysis", desc: "완비 내적공간." },
  { id: "InnerProdSpace", label: "내적공간",         type: "definition", category: "analysis", desc: "내적이 정의된 벡터공간." },
  { id: "TopGroup",       label: "위상군",           type: "definition", category: "geometry", desc: "위상공간이면서 군인 구조. 군 연산이 연속." },
  ...
  { id: "L2",     label: "L²(0,1)",   type: "instance", category: "analysis", desc: "[0,1] 위 제곱적분가능 함수. 무한차원 힐베르트 공간." },
  { id: "C01",    label: "C([0,1])",   type: "instance", category: "analysis", desc: "[0,1] 위 연속함수. sup 노름 바나흐 공간." },
  { id: "Zn",     label: "ℤ/nℤ",     type: "instance", category: "algebra",  desc: "나머지류군. 유한 순환군." },
  { id: "Poly_R", label: "ℝ[x]",     type: "instance", category: "algebra",  desc: "실수 계수 다항식환. 가환환, ℝ-대수." },
  { id: "Mat_n",  label: "M(n,ℝ)",   type: "instance", category: "algebra",  desc: "n×n 실행렬. n≥2 비가환환. ℝ-대수." },
  { id: "so3",    label: "𝔰𝔬(3)",    type: "instance", category: "algebra",  desc: "SO(3)의 리 대수. 3×3 반대칭행렬." },
  { id: "gl_n",   label: "𝔤𝔩(n,ℝ)", type: "instance", category: "algebra",  desc: "GL(n,ℝ)의 리 대수. n×n 실행렬 (교환자 괄호)." },
];

for (const n of nodes) {
  insertNode.run(n.id, n.label, n.type, n.category, n.desc, null);
}

// ── Edges ──
const insertEdge = db.prepare(
  'INSERT INTO edges (source, target, type, label, detail, detail_ref) VALUES (?, ?, ?, ?, ?, ?)'
);
const insertEdgeTag = db.prepare(
  'INSERT INTO edge_tags (edge_id, tag_id) VALUES (?, ?)'
);

const edges = [
  { source: "Set", target: "Monoid", type: "strengthen", tags: ["functor:free"], label: "연산·항등원 추가", detail: "추가: 결합적 이항연산, 항등원\n\n[functor:free] F: Set→Mon. F(S)=S* (유한 단어열)\n[functor:forgetful] U: Mon→Set. F⊣U 수반쌍." },
  ...
  { source: "SO3", target: "so3", type: "dual", tags: ["functor:lie"], label: "Lie(SO(3))", detail: "리 함자 인스턴스.\nLie(SO(3)) = 𝔰𝔬(3) = {A∈M₃(ℝ) | Aᵀ=−A}\n3차원, 외적과 리 대수 동형." },
  { source: "GL_n", target: "gl_n", type: "dual", tags: ["functor:lie"], label: "Lie(GL(n))", detail: "리 함자 인스턴스.\nLie(GL(n,ℝ)) = 𝔤𝔩(n,ℝ) = M(n,ℝ)\n괄호: [A,B]=AB−BA. 차원=n²." },
  // isomorphism
  { source: "HilbertSpace", target: "DualSpace", type: "isomorphism", tags: [], label: "H ≅ H*", detail: "리스 표현정리 (Riesz representation theorem)\nΦ: H → H*,  Φ(y)(x) = ⟨x,y⟩\n\nΦ는 반선형 등거리 동형사상.\n조건: H가 완비 내적공간일 것.\n모든 힐베르트 공간이 자기 쌍대와 동형." },
  { source: "Rn", target: "DualSpace", type: "isomorphism", tags: [], label: "ℝⁿ ≅ (ℝⁿ)*", detail: "유한차원 내적공간의 자기 쌍대.\n⟨x,y⟩=Σxᵢyᵢ에 의해 ℝⁿ ≅ (ℝⁿ)*.\n\n더 일반: 모든 유한차원 V에 대해 V ≅ V** (이중 쌍대, 자연 동형)." },
  // instance
  { source: "Z", target: "AbelianGroup", type: "instance", tags: [], label: "(ℤ,+)", detail: null },
  { source: "Z", target: "Ring", type: "instance", tags: [], label: "(ℤ,+,×)", detail: null },
  { source: "Q", target: "Field", type: "instance", tags: [], label: null, detail: null },
  { source: "R", target: "Field", type: "instance", tags: [], label: null, detail: null },
  { source: "C", target: "Field", type: "instance", tags: [], label: null, detail: null },
  { source: "Rn", target: "VectorSpace", type: "instance", tags: [], label: null, detail: null },
  { source: "Rn", target: "HilbertSpace", type: "instance", tags: [], label: "표준 내적", detail: "ℝⁿ에 ⟨x,y⟩=Σxᵢyᵢ 부여.\n유한차원 → 자동 완비 → 힐베르트 공간." },
  { source: "SO3", target: "LieGroup", type: "instance", tags: [], label: null, detail: null },
  { source: "GL_n", target: "LieGroup", type: "instance", tags: [], label: null, detail: null },
  { source: "S_n", target: "Group", type: "instance", tags: [], label: null, detail: "n개 원소의 치환 전체.\n|Sₙ|=n!. n≥3이면 비가환.\nS₃은 가장 작은 비가환군 (위수 6)." },
  { source: "L2", target: "HilbertSpace", type: "instance", tags: [], label: null, detail: "⟨f,g⟩=∫₀¹ f(t)g(t)dt.\n무한차원 분리가능 힐베르트 공간.\n완비: 르베그 적분에 의해." },
  { source: "C01", target: "BanachSpace", type: "instance", tags: [], label: "sup 노름", detail: "‖f‖∞=sup|f(t)|. 균등 수렴으로 완비.\n힐베르트가 아님: sup 노름은 내적 유도 아님." },
  { source: "Zn", target: "AbelianGroup", type: "instance", tags: [], label: "순환군", detail: "ℤ/nℤ = {0,...,n−1} mod n.\n|ℤ/nℤ|=n. p 소수이면 체." },
  { source: "Poly_R", target: "Ring", type: "instance", tags: [], label: null, detail: null },
  { source: "Poly_R", target: "Algebra_str", type: "instance", tags: [], label: "ℝ-대수", detail: null },
  { source: "Mat_n", target: "Ring", type: "instance", tags: [], label: null, detail: "행렬 덧셈·곱셈. n≥2 비가환.\n단위원: 단위행렬 I." },
  { source: "Mat_n", target: "Algebra_str", type: "instance", tags: [], label: "ℝ-대수", detail: null },
  { source: "so3", target: "LieAlgebra", type: "instance", tags: [], label: null, detail: null },
  { source: "gl_n", target: "LieAlgebra", type: "instance", tags: [], label: null, detail: null },
  // object-object strengthen
...
const insertAll = db.transaction(() => {
  for (const e of edges) {
    const info = insertEdge.run(e.source, e.target, e.type, e.label || null, e.detail || null, null);
    const edgeId = info.lastInsertRowid;
    if (e.tags) {
      for (const tag of e.tags) {
        insertEdgeTag.run(edgeId, tag);
      }
    }
  }
});
...
const counts = {
  ...
  edge_tags: db.prepare('SELECT count(*) as c FROM edge_tags').get().c,
  edge_types: db.prepare('SELECT count(*) as c FROM edge_types').get().c,
  tag_groups: db.prepare('SELECT count(*) as c FROM tag_groups').get().c,
  tags: db.prepare('SELECT count(*) as c FROM tags').get().c,
};
console.log('✓ math-kg.db created');
