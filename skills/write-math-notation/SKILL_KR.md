...
- 수식 구분자, 코드 포맷, Mermaid 라벨, Markdown 표 셀 때문에 수식 렌더링이 깨질 때
- 수식 비슷한 표기를 LaTeX로 쓸지, 유니코드 기호로 쓸지, 그냥 말로 풀어 쓸지 정할 때

## 렌더링 모델

- 본문 수식은 `remark-math`와 KaTeX로 렌더링합니다.
- Mermaid 라벨은 TeX를 렌더링하지 않습니다. plain text와 유니코드 기호만 처리합니다.
...
Mermaid 라벨에서 수식 비슷한 표기가 필요하면 `Δ`, `σ`, `∫`, `⊂`, `→` 같은 유니코드 기호를 사용합니다.

쓰지 않는 것:

- Mermaid 라벨 안에 `\Delta`, `\sigma`, `\int` 같은 TeX 명령 쓰기
- 실제 기호를 쓸 수 있는데 `Delta`, `sigma`처럼 plain text 단어로 풀어 쓰기
...
3. Mermaid 라벨이면 TeX 비슷한 표기를 유니코드 기호로 바꿉니다.
4. 표 안 수식이면 표 밖으로 빼거나 유니코드 기호로 다시 씁니다.
5. 문법이 맞는데도 계속 깨지면 사이트 렌더러를 확인합니다.
