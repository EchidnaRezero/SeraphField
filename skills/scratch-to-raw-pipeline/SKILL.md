...
3. Before anything enters `DRAFT/`, apply `AGENTS.md` for public-safety, frontmatter discipline, links, and parser-safe repository rules.
...
8. Before finishing, use `check-site-content`.
...
- Move content into `DRAFT/` only after it passes the public-safety bar for repository tracking, such as no personal identifiers, no local paths, and no private-only notes.
